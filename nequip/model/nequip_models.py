...
def FullNequIPGNNModel(
    ...
    # added key
    atomic_custom_key: Optional[str] = None,
    edge_custom_key: Optional[str] = None,
    # derivatives
    ...
    edge_norm = EdgeLengthNormalizer_mod(
        r_max=r_max,
        type_names=type_names,
    ...
    bessel_encode = BesselEdgeLengthEncoding(
        ...
        irreps_in=edge_type_embed.irreps_out,
    )
    ...
    for layer_i in range(num_layers):
        current_convnet = ConvNetLayer_mod(
    ...
    modules.update(
        {
            "per_atom_energy_redout": per_atom_energy_readout,
        }
    )

    prev_irreps_out = per_atom_energy_readout.irreps_out

    if atomic_custom_key is not None:
        atomic_custom_readout = ScalarMLP(
            output_dim=1,
            bias=False,
            forward_weight_init=True,
            field=AtomicDataDict.NODE_FEATURES_KEY,
            out_field=atomic_custom_key,
            irreps_in=prev_irreps_out,
        )
        modules.update(
        {
            "atomic_custom_readout": atomic_custom_readout,
        }
        )
    ...
    if edge_custom_key is not None:
        ...
        prev_irreps_out = edge_custom_readout.irreps_out
