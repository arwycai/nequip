...
from .interaction_block import InteractionBlock, InteractionBlock_mod
...
class ConvNetLayer_mod(GraphModuleMixin, torch.nn.Module):
    """
    Args:
    ...
    def __init__(
        ...
        convolution=InteractionBlock_mod,
        ...
        self._init_irreps(
            irreps_in=irreps_in,
            required_irreps_in=[AtomicDataDict.NODE_FEATURES_KEY, AtomicDataDict.EDGE_FEATURES_KEY],
        )

        # assumption: irreps of edge_feats and node_feats are the same!
        ...
        #TODO: update this pipeline for edge
        self.irreps_out[AtomicDataDict.NODE_FEATURES_KEY] = (
            self.equivariant_nonlin.irreps_out
        )
        self.irreps_out[AtomicDataDict.EDGE_FEATURES_KEY] = (
            self.equivariant_nonlin.irreps_out
        )
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        old_edge_x = data[AtomicDataDict.EDGE_FEATURES_KEY]
        # run convolution
        data = self.conv(data)
        # do nonlinearity
        ...
        data[AtomicDataDict.EDGE_FEATURES_KEY] = self.equivariant_nonlin(
        ...
        if self.resnet:
            ...
            data[AtomicDataDict.EDGE_FEATURES_KEY] = (
                old_x + data[AtomicDataDict.EGDE_FEATURES_KEY]
            )
        return data
