...
from ._tp_scatter_base import TensorProductScatter, TensorProductScatter_mod
from .norm import AvgNumNeighborsNorm

from typing import Sequence, Union, Dict

class InteractionBlock_mod(GraphModuleMixin, torch.nn.Module):
    use_sc: bool
    ...
    def __init__(
        ...
        #### important assumption that is supported by previous code: node_feats and edge_feats have same irrep
        ...
        self._init_irreps(
            ...
            irreps_out={AtomicDataDict.NODE_FEATURES_KEY: irreps_out, AtomicDataDict.EDGE_FEATURES_KEY: irreps_out},
        )
        ...
        self.tp_scatter = TensorProductScatter_mod(
            feature_irreps_in,
            irreps_edge_attr,
            irreps_mid,
            instructions,
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        edge_x = data[AtomicDataDict.EDGE_FEATURES_KEY]
        ...
        x = self.linear_1(x)
        edge_x = self.linear_1(edge_x)

        # normalize before TP-scatter
        # not normalizing edge since there is no sum of edges for each edge
        ...
        x = self.linear_2(x)
        edge_x = self.linear_2(edge_x)

        if self.sc is not None:
            x = x + sc

        data[AtomicDataDict.NODE_FEATURES_KEY] = x
        data[AtomicDataDict.EDGE_FEATURES_KEY] = edge_x
